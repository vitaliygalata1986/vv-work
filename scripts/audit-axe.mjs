...
try {
  for (const width of [1440, 768, 375]) {
    ...
    await page.goto(`${baseURL}/partners/vv-work`)
    await page.getByText('Показано 10 із 18', { exact: true }).waitFor()
    assert.equal(await page.getByRole('article').count(), 10)
    await scan(page, `jobs-first-batch-${width}`)
    await page.getByRole('button', { name: 'Показати ще', exact: true }).click()
    await page.getByText('Показано 18 із 18', { exact: true }).waitFor()
    assert.equal(await page.getByRole('article').count(), 18)
    assert.equal(await page.getByRole('button', { name: 'Показати ще', exact: true }).count(), 0)
    assert.equal(await page.getByRole('list', { name: 'Список вакансій' })
      .locator(':scope > li').nth(10).evaluate(element => element === document.activeElement), true)
    await scan(page, `jobs-expanded-${width}`)
